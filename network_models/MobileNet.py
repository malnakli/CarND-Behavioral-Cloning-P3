...
def model(weights=True,freez_pertrian_layers=True):
    inputs = Input(shape=(200, 200, 3), name='mobilenet_input')
    op = Lambda(lambda x: (x / 255.0) - 0.5)(inputs)
    # and input must have a static square shape
    app_model = MobileNet(include_top=False,
                        weights='imagenet',input_shape=(200,200,3), input_tensor=op,dropout=1e-3)
